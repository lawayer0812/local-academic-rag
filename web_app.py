...
PAPERS_FOLDER = os.path.join(PROJECT_DIR, CONFIG.get("papers_folder", "papers"))
CHROMA_FOLDER = os.path.join(PROJECT_DIR, CONFIG.get("chroma_folder", "chroma_db"))
...
def ensure_pdf_server():
    ...
    try:
        subprocess.Popen([
            sys.executable, "-m", "http.server", str(active_port),
            "--bind", "127.0.0.1", "--directory", PAPERS_FOLDER,
...
def build_pdf_url(source, page):
    ...
    if ACTIVE_PDF_PORT is None:
        return "#"
    return f"http://127.0.0.1:{ACTIVE_PDF_PORT}/{encoded_name}#page={page}"
    ...
    for result in trace_results:
        ...
        for part in parts[1:]:
            ...
            if ref_text and ref_text != "未找到":
                matched_blocks.append(f"**完整参考文献：** {ref_text}")
    unique = []
    seen = set()
...
def insert_reference_matches(answer, matches):
    ...
    if usage_heading in answer:
        ...
        return before.rstrip() + refs_block + "\n" + usage_heading + after
        ...
        if st.button("➕ 加入知识库", type="primary"):
            ...
            if os.path.exists(save_path):
                ...
                if result.returncode == 0:
                    # app.py writes ChromaDB in a child process. The running
                    # Streamlit process may still hold an old Rust/HNSW reader.
                    # Request a real process restart so the database is reopened
                    # cleanly before the user searches the newly added paper.
                    restart_flag = os.path.join(PROJECT_DIR, ".rag_restart_requested")
                    with open(restart_flag, "w", encoding="utf-8") as f:
                        f.write("restart\n")
                    st.success("✅ 新论文已经成功加入知识库！")
                    st.info("正在刷新知识库服务，请稍候几秒后继续检索。")
                    time.sleep(1.0)
                    os._exit(42)
                else:
...
def get_sources():
    ...
    return sorted(source_set, key=lambda source: (-get_publication_year(source), source.lower()))


def render_library_directory(sources):
    st.subheader("📚 当前文献库目录")
    st.caption(f"共 {len(sources)} 篇论文，按文件名中的年份从近到远排列。点击文献名可直接打开 PDF。")
    if not PDF_SERVER_OK:
        st.warning(f"PDF 浏览服务未能自动启动（端口 {PDF_PORT}）。请重新启动应用后再试。")
    if not sources:
        st.info("当前文献库中还没有论文。")
        return
    for i, source in enumerate(sources, start=1):
        ...
        with col_title:
            ...
            if os.path.exists(pdf_path):
                st.markdown(f'<a href="{build_pdf_url(source, 1)}" target="_blank">{label}</a>', unsafe_allow_html=True)
            else:
                st.write(label)
...
with col1:
    ...
    st.markdown(f"""
        <a href="{library_href}" target="_self" style="text-decoration:none;color:inherit;">
            <div style="border:1px solid rgba(128,128,128,0.25);border-radius:0.5rem;padding:0.75rem 1rem;min-height:88px;">
                <div style="font-size:0.875rem;color:rgba(120,120,120,0.95);">论文数量（点击查看目录）</div>
                <div style="font-size:2rem;font-weight:600;line-height:1.35;">{paper_count}</div>
            </div>
        </a>
...
# =========================================================
st.subheader("检索设置")
mode = st.radio("选择检索模式", ["全库检索", "指定文献检索"], horizontal=True)
source_filter = None
if mode == "指定文献检索":
...
if search_button:
    ...
    with st.spinner("正在知识库中检索相关文献……"):
        retrieved = retrieve(query=query, source_filter=source_filter)
    if not retrieved:
    ...
    with st.spinner("DeepSeek 正在核对并整理最相关证据……"):
        try:
            ...
            reference_matches = extract_successful_reference_matches(trace_results)
            answer = insert_reference_matches(answer, reference_matches)
        except Exception as e:
...
if "rag_retrieved" in st.session_state:
    ...
    st.divider()
    st.subheader("🔎 原始检索位置")
    st.caption("以下为 BGE-M3 找到的主要相关位置。点击“打开原文”可在浏览器中直接核对对应 PDF 页码；DeepSeek 会继续结合相邻文本筛选证据。")
    main_results = [item for item in retrieved if item.get("type") == "main"]
    for i, item in enumerate(main_results, start=1):
        with st.container(border=True):
            ...
            if os.path.exists(pdf_path):
                st.link_button(f"📖 打开原文第 {item['page']} 页", build_pdf_url(item["source"], item["page"]))
            else:
